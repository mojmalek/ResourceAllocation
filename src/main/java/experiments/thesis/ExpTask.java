    ...
    public static void smallWorldSim() throws StaleProxyException, IOException {
        ...
        for (int exp = 1; exp <= 5; exp++) {
            ...
            for (long param = 1; param <= 4; param += 1) {
                ...
                simEngTask = new SimEngTask( param, agentType, 4, 8, 2);
                simEngTask.maxResourceTypesNum = 2;
...
//              profile.setParameter(Profile.GUI, "true");
                ...
                for (int i = 0; i <= numberOfAgents; i++) {
                    ...
                    try {
                        if (i == 0) {
                            agentController = containerController.createNewAgent(agentType + i, "agents.DeepRLMasterAgent", new Object[]{numberOfAgents, numberOfEpisodes, smallWorldGraph, smallWorldAdjacency, logFileMaster, resultFileCen, resultFileDec, agentType, simEngTask.maxTaskNumPerAgent, simEngTask.resourceTypesNum, simEngTask.maxResourceTypesNum, trainedModelPath, packageSize});
                            agentController.start();
                        } else {
                            agentController = containerController.createNewAgent(agentType + i, "agents.DeepRLSocialAdaptiveAgent", new Object[]{numberOfAgents, numberOfEpisodes, smallWorldGraph, smallWorldAdjacency[i - 1], logFileAll, simEngTask, agentType, simEngTask.maxRequestQuantity, trainedModelPath, packageSize});
                            agentController.start();
                        }
...
//              profile.setParameter(Profile.GUI, "true");
                ...
                for (int i = 0; i <= numberOfAgents; i++) {
                    ...
                    try {
                        if (i == 0) {
                            agentController = containerController.createNewAgent(agentType + i, "agents.DeepRLMasterAgent", new Object[]{numberOfAgents, numberOfEpisodes, scaleFreeGraph, scaleFreeAdjacency, logFileMaster, resultFileCen, resultFileDec, agentType, simEngTask.maxTaskNumPerAgent, simEngTask.resourceTypesNum, simEngTask.maxResourceTypesNum, trainedModelPath, packageSize});
                            agentController.start();
                        } else {
                            agentController = containerController.createNewAgent(agentType + i, "agents.DeepRLSocialAdaptiveAgent", new Object[]{numberOfAgents, numberOfEpisodes, scaleFreeGraph, scaleFreeAdjacency[i - 1], logFileAll, simEngTask, agentType, simEngTask.maxRequestQuantity, trainedModelPath, packageSize});
                            agentController.start();
                        }
                        agentControllers.add( agentController);
                    } catch (StaleProxyException e) {
...
//                containerController.kill();
    ...
    public static void randomSim() throws StaleProxyException, IOException {
        ...
        for (int exp = 1; exp <= 5; exp++) {
            ...
            String trainedModelPath = "trained_models/task" + numberOfAgents + "random" + exp;
...
//              profile.setParameter(Profile.GUI, "true");
                ...
                for (int i = 0; i <= numberOfAgents; i++) {
                    ...
                    try {
                        if (i == 0) {
                            agentController = containerController.createNewAgent(agentType + i, "agents.DeepRLMasterAgent", new Object[]{numberOfAgents, numberOfEpisodes, randomGraph, randomAdjacency, logFileMaster, resultFileCen, resultFileDec, agentType, simEngTask.maxTaskNumPerAgent, simEngTask.resourceTypesNum, simEngTask.maxResourceTypesNum, trainedModelPath, packageSize});
                            agentController.start();
                        } else {
                            agentController = containerController.createNewAgent(agentType + i, "agents.DeepRLSocialAdaptiveAgent", new Object[]{numberOfAgents, numberOfEpisodes, randomGraph, randomAdjacency[i - 1], logFileAll, simEngTask, agentType, simEngTask.maxRequestQuantity, trainedModelPath, packageSize});
                            agentController.start();
                        }
                        agentControllers.add( agentController);
                    } catch (StaleProxyException e) {
